#!/usr/bin/env python3
"""
Database Transaction Monitor: Reads real transaction data from frontend database
Captures actual transaction amounts from the database
"""
...
class DatabaseTransactionMonitor:
    ...
    def setup_agent_handlers(self):
        ...
        print("🎯 Reading real transaction data from database")
    ...
    def get_latest_transaction(self):
        ...
        try:
            # Try to get transaction data from the ledger-db
            result = subprocess.run([
                "kubectl", "exec", "deployment/ledger-db", "--", 
                "psql", "-U", "postgres", "-d", "postgres", 
                "-c", "SELECT * FROM transactions ORDER BY timestamp DESC LIMIT 1;"
    ...
    def monitor_database_transactions(self):
        ...
        print("👀 Monitoring database for new transactions...")
        ...
        try:
            while True:
                ...
                if db_output:
                    ...
                    if transaction_data:
                        print(f"💳 New transaction found in database:")
...
def main():
    """Main function"""
    demo = DatabaseTransactionMonitor()
