#!/usr/bin/env python3
"""
Auto-Detect Frontend Monitor: Automatically detects transaction amounts from frontend logs
Only reacts to real user transactions with automatic amount detection
"""
...
class AutoDetectFrontendMonitor:
    def __init__(self):
    ...
    def setup_agent_handlers(self):
        ...
        print("🎯 Auto-detecting YOUR real transactions")
    ...
    def extract_amount_from_log(self, log_line: str):
        """Extract transaction amount from log line"""
        try:
                ...
                for log_line in logs:
                    if log_line and log_line not in self.last_logs:
                        ...
                        if self.is_new_transaction(log_line):
                            ...
                            # Try to extract amount automatically
                            amount = self.extract_amount_from_log(log_line)
                            
                            if amount is not None:
                                print(f"   Auto-detected amount: ${amount:,.2f}")
                            else:
                                print("   Could not auto-detect amount")
...
def main():
    """Main function"""
    demo = AutoDetectFrontendMonitor()
